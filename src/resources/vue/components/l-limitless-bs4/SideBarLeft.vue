                                ...
                                <tr v-for="txn in tableData.payload.data"
                                    ...
                                    :class="[txn.id == $route.params.id ? 'table-warning border-left-3 border-warning' : '']">
                                        <td class="col-md-6 cursor-pointer">
                                            <h6 class="m-0">
                                                <div class="rg-nowrap-ellipsis">{{txn.reference}}</div>
                                        ...
                                        <td class="col-md-6 cursor-pointer">
                                            <div class="text-right">
                                                <div class="rg-nowrap-ellipsis">
                                                    {{rgNumberFormat(txn.total, 2)}} <small>{{txn.currency}}</small>
                                                </div>
    ...
    export default {
        ...
        watch: {
            '$route.query.page': function (page) {
                this.tableData.url = '/journal-entries/' + '?page=' + page;
                this.$root.LoadingTxnSideBarLeft = true
            }
        },
        mounted() {

            let rgTableDataUrl = '/journal-entries'
        ...
        methods: {
            onRowClick(txn) {
                ...
                let path = '/journal-entries/' + txn.id
